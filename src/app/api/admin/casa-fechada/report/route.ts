...
export async function POST(req: NextRequest) {
  try {
    ...
    const html = buildHtml({ items, title: title || 'A Casa Fechada – Relatório de Pagamentos', generatedAt: generatedAt || new Date().toLocaleString('pt-BR'), logoDataUrl });
    ...
    return new NextResponse(Buffer.from(pdf), { headers: { ...corsHeaders, 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="casa-fechada-relatorio.pdf"' } });
  } catch (e) {
