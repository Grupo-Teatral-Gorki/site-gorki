...
export default function AdminDesventurasPage() {
  ...
  const downloadPDF = async () => {
    ...
    try {
      const res = await fetch('/api/admin/desventuras/report', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          items: filteredItems,
          generatedAt: new Date().toLocaleString('pt-BR'),
          title: 'Desventuras – Relatório de Pagamentos'
        }),
      });
      ...
      a.href = url;
      a.download = 'desventuras_relatorio.pdf';
      a.click();
  ...
  const filteredItems = useMemo(() => {
    const byTitle = (i: Item) => {
      if (!titleFilter) return true;
      const hay = `${i.eventTitle || ''} ${i.eventId || ''}`.toLowerCase();
      return hay.includes(titleFilter.toLowerCase());
    };
    return items.filter(byTitle);
  }, [items, titleFilter]);
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8 px-4 text-gray-900">
      <div className="max-w-7xl mx-auto bg-white/90 backdrop-blur rounded-2xl shadow-lg border border-gray-200 p-5 sm:p-7">
        ...
        {/* Filter: Text-based event filter */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          <div className="sm:col-span-3">
            <label className="block text-xs text-gray-600 mb-1">Evento (título ou ID)</label>
            <input
              type="text"
              value={titleFilter}
            ...
            <table className="min-w-full text-sm whitespace-nowrap">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr className="text-left">
                  ...
                  <th className="px-3 py-2 font-semibold text-gray-700">Evento</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredItems.map((i) => (
                  <tr key={i.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">{i.eventTitle || i.eventId}</td>
